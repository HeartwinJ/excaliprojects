...
import { listTagsForBoards } from "../../repos/tags.js";
...
/** Attach a `tags` array to each board summary in a single batched query. */
export async function withBoardTags(
  boards: BoardSummary[],
  ownerId: string
): Promise<Array<BoardSummary & { tags: { id: string; name: string }[] }>> {
  const tagsByBoard = await listTagsForBoards(
    boards.map((b) => b.id),
    ownerId
  );
  return boards.map((b) => ({
    ...b,
    tags: tagsByBoard.get(b.id) ?? [],
  }));
}
...
boardsRouter.post(
  ...
  async (req, res) => {
    const parsed = z
      .object({ name: nameSchema.default("Untitled") })
      .safeParse(req.body ?? {});
    if (!parsed.success) {
    ...
    res.status(201).json({ ...board, tags: [] });
  }
);
...
boardsRouter.patch(
  ...
  async (req, res) => {
    ...
    if (parsed.data.isFavorite !== undefined) {
      summary =
        (await setBoardFavourite(boardId, userId, parsed.data.isFavorite)) ??
        summary;
    }
    ...
    const [withTags] = await withBoardTags([summary], userId);
    res.json(withTags);
  }
);
...
boardsRouter.put(
  ...
  async (req, res) => {
    ...
    const updated = await saveBoardScene(
      req.params.id!,
      req.session.userId!,
      scene
    );
    ...
    void maybeAutoSnapshot(updated.id, scene, updated.save_count).catch(
      () => undefined
    );
